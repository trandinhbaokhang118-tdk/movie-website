...
async function ManagedTitlePage({ title }: { title: ManagedTitle }) {
  ...
  return <main className="title-experience"><SiteHeader /><section className="detail-hero managed-detail-hero detail-hero-premium" style={safePoster ? { backgroundImage: `linear-gradient(90deg, rgba(5,4,11,.98) 0%, rgba(5,4,11,.72) 48%, rgba(5,4,11,.35)), url("${safePoster}")` } : undefined}><CinematicDepth /><div className="page-shell detail-content"><Link className="back-link" href="/browse">← Trở lại thư viện</Link><p className="hero-kicker"><span>CINEWAVE</span> {isUpcoming ? "SẮP RA MẮT" : "MỚI PHÁT HÀNH"}</p><h1>{title.title}</h1><p className="hero-original">{title.originalTitle}</p><div className="title-meta"><strong>{isUpcoming && title.scheduledAt ? `Ra mắt ${new Date(title.scheduledAt).toLocaleString("vi-VN")}` : "Đã kiểm duyệt"}</strong><span>{title.releaseYear}</span><span className="maturity-badge">{title.maturity}</span><span>{title.duration}</span><span>{title.contentType === "series" ? "Series" : "Phim lẻ"}</span></div><p className="detail-synopsis">{title.synopsis}</p><div className="hero-actions">{!isUpcoming && title.videoUrl ? <Link className="button button-cinema" href={`/watch/${title.id}`}><span aria-hidden="true">▶</span> Xem phim</Link> : <span className="button button-secondary" aria-disabled="true">{isUpcoming ? "Chưa đến ngày phát hành" : "Đang xử lý media"}</span>}<WatchlistButton movieId={title.id} initialSaved={saved} signInUrl={signInPath(`/title/${title.id}`)} /></div></div></section><div className="page-shell detail-body detail-bento-body"><section className="detail-facts detail-facts-bento" aria-label="Thông tin phim"><div><p>Thể loại</p><strong>{title.genres}</strong></div><div><p>Phân loại</p><strong>{title.maturity}</strong></div><div><p>Nguồn phát</p><strong>{isUpcoming ? "Đã khóa đến ngày ra mắt" : title.videoUrl ? "Media đã đăng ký" : "Đang xử lý"}</strong></div><div><p>Trạng thái</p><strong>{isUpcoming ? "Sắp ra mắt" : "Đã phát hành"}</strong></div></section><section className="rights-notice"><div><p className="eyebrow">MINH BẠCH BẢN QUYỀN</p><h2>{title.licenseName}</h2></div><p>Bản ghi được tạo bởi {title.createdBy}. <a className="text-link" href={title.licenseUrl} target="_blank" rel="noreferrer">Xem giấy phép nguồn ↗</a></p></section></div><Footer /></main>;
}
